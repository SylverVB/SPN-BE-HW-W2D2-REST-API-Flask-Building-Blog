...
class TestPostEndpoints(unittest.TestCase):
    ...
    def test_get_all_posts(self, mock_get, mock_scalars, mock_current_user, mock_verify_token):
        ...
        mock_posts = [Post(post_id=fake.random_int(min=1, max=1000),
                        ...
                        user_id=mock_user.user_id) for _ in range(5)]
        mock_scalars.return_value.all.return_value = mock_posts

        logger.debug(f"Mock scalars return value: {mock_scalars.return_value.all()}")
        token = encode_token(mock_user.user_id)
...
class TestCommentEndpoints(unittest.TestCase):
    ...
    @patch('app.routes.db.session.scalars')
    @patch('app.routes.db.session.get')
    def test_list_comments_for_post(self, mock_get, mock_scalars, mock_current_user, mock_verify_token):
        ...
        # Set up mock comment data with Faker
        mock_comments = [Comment(comment_id=fake.random_int(min=1, max=1000),
                                post_id=1,
                                content=fake.paragraph(nb_sentences=2)) for _ in range(5)]
        mock_scalars.return_value.all.return_value = mock_comments
